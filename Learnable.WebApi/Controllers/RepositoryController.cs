...
namespace Learnable.WebApi.Controllers
{
    ...
    public class RepositoryController : BaseController
    {
        ...
        // GET: api/Repository
        [HttpGet]    // http://localhost:5071/api/Repository
        public async Task<ActionResult<IEnumerable<RepositoryDtos>>> GetAll()
        {
            var query = new GetAllRepositoryQuery();
        ...
        {
            var query = new GetRepositoryByIdQuery(id);
            var result = await _mediator.Send(query);
        ...
        // POST: api/Repository
        [HttpPost]
        public async Task<ActionResult<RepositoryDtos>> Create([FromBody] CreateRepositoryDto dto)
        {
            if (dto == null)
                return BadRequest("Repository data is required");

            var command = new CreateRepositoryCommand(dto);
            var result = await _mediator.Send(command);

            return CreatedAtAction(nameof(GetById), new { id = result.RepoId }, result);
        }
